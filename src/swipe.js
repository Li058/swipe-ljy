...
Swipe.prototype.addEvent = function() {
    ...
    this.device = (/android | iphone | iad | webos | blackberry/i.test(window.navigator.userAgent.toLowerCase()));
    ...
    // 添加鼠标点击或手机点击事件
    var that = this;
    cas.addEventListener(this.clickEvent, function(evt) {
    ...
    cas.addEventListener(this.moveEvent, function(evt) {
        if (!that.isMouseDown) {
            return false;
        } else {
            var event = evt || window.event;
...
Swipe.prototype.drawLine = function(x1, y1, x2, y2) {
    ...
    this.context.lineCap = "square"; // 连接点效果为圆的
